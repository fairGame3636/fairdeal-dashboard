...
        receiveBy: '', // Only receiveBy is used in this report's filter
        // sendBy is not needed for OutPoints report filter
        startDate: '',
        endDate: '',
...
                // Fetch roles supported by UserService.listUser (exclude 'admin' because the service type doesn't accept it)
                const rolesToFetch: ('agent' | 'sub-agent' | 'user')[] = ['agent', 'sub-agent', 'user']; // Fetch allowed roles for 'Receive By'
...
                     setApiError("Could not load user list for 'Receive By' filter.");
                 } else if (fetchError) {
...
    const handleClear = () => {
        setFilters({ receiveBy: '', startDate: '', endDate: '', dateRange: undefined });
        setTableData([]);
...
            receiveBy: filters.receiveBy || undefined, // Send username or undefined
...
        // Assuming ReportService.getOutPoints uses the same LedgerParams structure
        const response = await ReportService.getOutPoints(params);

        if (response.success && response.data) {
            setTableData(response.data.records || []);
            setPaginationData(response.data.pagination);
             // Ensure the key matches the backend response (e.g., totalOUT)
            setTotalOut(response.data.totalOUT || 0);
        } else {
            setApiError(response.message || 'Failed to fetch Out Points report.');
            setTableData([]);
            setPaginationData(null);
            setTotalOut(0); // Reset total on error
        }
        setIsLoading(false);
    };
...
         setApiError(null);
         try {
              // Pass current filters (including username from dropdown)
...
                   dateRange: filters.dateRange ? (filters.dateRange as any) : undefined,
             };
             const fileBlob = await ReportService.exportOutPoints(paramsToExport);
             downloadFile(fileBlob, 'Out_Points_Report.xlsx');
         } catch (err) {
...
            <h1 className="text-3xl font-bold text-green-800 mb-6">Out Points Report</h1>
...
                <div className="bg-white p-6 rounded-xl shadow-lg border border-green-100">
                   {/* --- Total Display (COMMENTED OUT as per new plan) --- */}
                    {/* <div className="mb-4 text-md font-bold text-green-800">
                       TOTAL: OUT (<span className="text-lg text-red-500">{(totalOut || 0).toLocaleString()}</span>)
                    </div> */}
...
                                 <tr>
                                      {['S.No', 'Trans. Id', 'Date', 'Sender', 'Old Points', 'Out', 'New Points', 'Receiver', 'Comments'].map(header => (
...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{t.SENDER || '-'}</td>
...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{t.RECEIVER || <span className="px-2 py-1 text-xs rounded-full bg-gray-100 text-gray-600">System</span>}</td>
...
                                                Total:
                                 </td>
                                            {/* Empty cells to push value to the correct column (4 columns: Trans Id, Date, Sender, Old Points) */}
                                           <td colSpan={4} className="px-6 py-4"></td>

                                            {/* Out Total Value - Aligned under 'Out' column */}
                                     <td className="px-6 py-4 whitespace-nowrap text-sm text-red-600 font-bold">
                                                {(totalOut || 0).toLocaleString()}
                                     </td>

                                            {/* Empty cells for remaining columns (3 columns: New Points, Receiver, Comments) */}
...
export default OutPointsReport;