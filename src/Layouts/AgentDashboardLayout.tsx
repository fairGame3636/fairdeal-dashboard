...
const agentSidebarOptions = [
    ...
    },
    {
      ...
      options: [
          { label: 'Search Users', to: Strings.SearchUser },
    ...
    },
    {
      ...
      options: [
          { label: 'Create User', to: Strings.CreateSubAgent },
                            ...
                            <div key={group.groupLabel} className="mb-4">
                                ...
                                <div className="space-y-1">
                                    {group.options.map((option) => (
                                        <NavLink key={option.label} to={option.to} onClick={() => { if(option.to === '/agent/profile') { setIsProfileModalOpen(true); } setIsSidebarOpen(false) }} end={option.to === '/agent'} className={({ isActive }) => `w-full flex items-center pl-6 pr-4 py-2.5 rounded-lg text-base font-medium transition-all duration-200 group ${isActive && option.to !== '/agent/profile' ? 'bg-gradient-to-r from-green-400 to-green-500 text-white shadow-md' : 'text-green-700 hover:bg-green-200/50'}`}>
