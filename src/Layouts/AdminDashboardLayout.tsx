...
const adminSidebarOptions = [
    // ... sidebar options remain unchanged
    {
      groupLabel: 'Main',
      options: [
          { label: 'Dashboard', to: '/dashboard' },
          { label: 'GameUI', to: strings.GameUI },
          { label: 'Decide-Result', to: strings.DecideResult }
      ],
    },
    {
      ...
      options: [
          ...
          { label: 'Update Commission', to: strings.UpdateCommission}
      ],
    },
    {
      groupLabel: 'Admin Management',
      options: [
          { label: 'Create User', to: strings.CreateAgentOrSubagent },
    ...
    }, 
    {
        ...
        options: [
            { label: 'Update Version', to: strings.VersionUpdate }
        ]
    }
];
...
function DashboardLayout() {
    ...
    const [currentUser, setCurrentUser] = useState<any>(null); // State to hold user data
    const navigate = useNavigate();

    // Fetch real user data (Unchanged)
    useEffect(() => {
        const fetchProfile = async () => {
            ...
            if (response.success && response.data) {
                ...
                // Optionally handle logout or show error message
            }
        };
    ...
    return (
        <div className="w-full h-screen bg-white flex flex-col overflow-hidden">
            {/* --- Header (Updated to show dynamic data AND LOGO) --- */}
            <header className="flex items-center justify-between h-20 px-4 sm:px-6 bg-white border-b-2 border-green-200 shadow-sm flex-shrink-0 z-40">
                ...
                <div className="flex items-center space-x-2 sm:space-x-4">
                    {/* ---
                      ...
                      I added `hidden sm:block` to this div.
                      It hides the "Welcome" text on `xs` screens (mobile)
                      and shows it on `sm` screens (tablet) and up.
                      ---
                    */}
                    <div className="text-right hidden sm:block">
                        <p className="text-green-700 font-semibold text-sm sm:text-base whitespace-nowrap">Welcome, {currentUser?.username || '...'}</p>
                    </div>
                    <button onClick={() => setIsProfileModalOpen(true)} className="p-2 rounded-full hover:bg-gray-100 flex items-center justify-center sm:px-4 sm:py-2 font-bold text-white transition-all duration-200 transform shadow-lg bg-gradient-to-r from-green-500 to-green-600 hover:scale-105" title="View Profile">
                        {/* --- Changed icon size for mobile --- */}
                        <svg className="w-6 h-6 sm:w-7 sm:h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                    </button>
                    <button onClick={handleLogout} className="flex items-center justify-center p-2 sm:px-4 sm:py-2 font-bold text-white rounded-xl transition-all duration-200 transform shadow-lg bg-gradient-to-r from-green-500 to-green-600 hover:scale-105" title="Logout">
                        {/* --- Changed icon size for mobile --- */}
                        <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 sm:w-6 sm:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
                            ...
                            <div key={group.groupLabel} className="mb-4">
                                ...
                                <div className="space-y-1">
                                    {group.options.map((option) => (
                                        <NavLink key={option.label} to={option.to} onClick={() => setIsSidebarOpen(false)} end={option.to === '/dashboard'} className={({ isActive }) => `w-full flex items-center pl-6 pr-4 py-2.5 rounded-lg text-base font-medium transition-all duration-200 group ${isActive ? 'bg-gradient-to-r from-green-400 to-green-500 text-white shadow-md' : 'text-green-700 hover:bg-green-200/50'}`}>
